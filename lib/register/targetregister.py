...
class TargetRegister(BaseRegister):
    ...
    async def register_target(self):
        ...
        if conf.scan.simple:
            ...
            async for target in self.register_host(id_no, conf.scan.simple):
                yield target
            ...
            if os.path.isfile(conf.scan.file):
                ...
                with open(conf.scan.file, 'r') as f:
                    for line in f.readlines():
                        ...
                        if line and line != '':
                            async for target in self.register_host(id_no, line):
                                yield target
            else:
                sys.exit(log.error(f"Target file not exist: {conf.scan.file}..."))
                ...
                for host in root.findall('host'):
                    ...
                    for port in host.iter('port'):
                        ...
                        if port_state.lower() not in ['closed', 'filtered']:
                            ...
                            async for target in self.register_host(id_no, address, port_service):
                                yield target
            else:
                sys.exit(log.error(f"Target file not exist: {conf.scan.nmap_xml}..."))
        ...
        if conf.scan.task:
            ...
            for address in address_list:
                async for target in self.register_host(id_no, address):
                    yield target
        ...
        if conf.scan.search_engine:
            ...
            async for item in get_search_engine_api(conf.scan.search_engine):
                async for target in self.register_host(id_no, item):
                    yield target
                ...
                try:
                    ...
                    if match:
                        ...
                        if start_port > 0 and start_port < 65536 and end_port > 0 and end_port < 65536:
                            ...
                            sys.exit(log.error("Illegal input: %s" % port_scope))
                    else:
                        sys.exit(log.error("Illegal input: %s" % port_scope))
    ...
    async def register_host(self, no, target, service=None):
        # http://localhost
